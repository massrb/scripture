...
The insertions happen by looking for all rows that have WEBUS in the scriptureIndex. From each of those rows, the particular verse for the target bible version is derrived and the api.bible url for the API call is derrived to obtain the scripture needed for each row. 
